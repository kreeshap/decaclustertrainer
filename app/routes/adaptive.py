...
def _derive(user_id, event_id):
    ...
    topic_analysis = _analytics(questions, responses, catalog, mastery)
    total = len({k.get("code") for k in catalog if k.get("code")})
    studied = len({r.get("kpi_code") for r in completions if r.get("kpi_code")})
    ...
    correct = sum(bool(r.get("correct")) for r in responses)
    ...
    state = {
        ...
        "recent_question_accuracy": round(correct / len(responses), 3) if responses else None,
        ...
        "practice_attempt_count": len(responses),
...
def today_plan():
    ...
    if saved and not regenerate and (saved[0].get("inputs") or {}).get("planner_version") == 2:
        row = saved[0]
    ...
    inputs = {"planner_version": 2, "coverage": state["coverage"], "due_reviews": state["due_review_count"], "practice_attempts": state["practice_attempt_count"]}
